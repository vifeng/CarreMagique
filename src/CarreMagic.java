...
public class CarreMagic{
    public static void main(String[] args){
        ...
        Terminal.ecrireString(
                "Remplissez le tableau et vérifier si votre carré est magique. " +
                "La première colonne et première ligne commence à 0. " +
                "Vous ne pouvez pas utiliser un nombre qui a déjà été entré.");
        Terminal.sautDeLigne();
    ...
    for (int i = 0; i < carre.length; i++) {
        ...
        for (int j = 0; j < carre[carre.length-1].length; j++) {
            som3 += carre[i][j];//addition pour le total par ligne
            som4 += carre[j][i];//addition pour le total par colonne
        }
